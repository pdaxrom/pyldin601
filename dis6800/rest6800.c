...
uint16_t getlabels(struct symbol **syms, uint8_t *mem, uint16_t addr, uint8_t *map)
{
    opcodeinfo *op;
...
uint16_t disassembly(FILE *out, struct symbol *syms, uint8_t *mem, uint16_t addr, uint8_t *map)
{
    opcodeinfo *op;
...
int main(int argc, char *argv[])
{
    ...
    int c;
    uint16_t addr, startaddr = 0, length;
    int use_map = 0;
	...
	while (addr < startaddr + length) {
	    ...
	    addr = getlabels(&sym, MEM, addr, use_map ? MAP : NULL);
	    if (sym) {
		if (!syms) {
		    syms = sym;
		} else {
...
//	    fprintf(dstFile, "%04X\t", addr);
	    addr = disassembly(dstFile, syms, MEM, addr, use_map ? MAP : NULL);
	}
	fprintf(dstFile, "\n\n\n");
	break;
    }
