...
static opcodeinfo opcodes[]={
	...
	{ 0x16,	0,	"tab",	INH,	2 },
	{ 0x17,	0,	"tba",	INH,	2 },
	{ 0x18,	0,	"xgdx",	INH,	2 }, //FIXME
	{ 0x19,	0,	"daa",	INH,	2 },
	{ 0x1A,	0,	"slp",	INH,	2 }, //FIXME
	{ 0x1B,	0,	"aba",	INH,	2 },
	{ 0x20,	1,	"bra",	REL,	3 },
	{ 0x21,	1,	"brn",	REL,	3 },
	{ 0x22,	1,	"bhi",	REL,	3 },
...
void exportlist(FILE *out, struct symbol *syms, uint32_t start_addr, uint32_t end_addr)
{
	...
	if ((syms->addr < start_addr) || (syms->addr >= end_addr)) {
	    if (syms->addr < 0x100) {
		fprintf(out, "L_%02X  \t\tEQU\t$%02X\n", syms->addr, syms->addr);
	    } else {
		fprintf(out, "L_%04X\t\tEQU\t$%04X\n", syms->addr, syms->addr);
	    }
	}
	syms = syms->prev;
    }
